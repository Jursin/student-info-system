...
class StudentCreateForm(FlaskForm):
    ...
    phone = StringField("电话号码", validators=[DataRequired(), Regexp(r"^1\d{10}$", message="手机号为1开头11位数字")])
    major = SelectField("专业", choices=[("物理学", "物理学"), ("光电信息科学与工程", "光电信息科学与工程"), ("量子信息科学", "量子信息科学")], validators=[DataRequired()])
    clazz = StringField("班级", validators=[DataRequired(), Regexp(r"^\d{5}$", message="班级为5位数字")])
    hometown = StringField("籍贯", validators=[DataRequired(), Length(max=100)])
...
class StudentEditForm(FlaskForm):
    ...
    gender = SelectField("性别", choices=[("男", "男"), ("女", "女")], validators=[DataRequired()])
    id_card = StringField("身份证号码", validators=[DataRequired()])
    phone = StringField("电话号码", validators=[DataRequired(), Regexp(r"^1\d{10}$")])
    major = SelectField("专业", choices=[("物理学", "物理学"), ("光电信息科学与工程", "光电信息科学与工程"), ("量子信息科学", "量子信息科学")], validators=[DataRequired()])
    clazz = StringField("班级", validators=[DataRequired(), Regexp(r"^\d{5}$")])
    hometown = StringField("籍贯", validators=[DataRequired(), Length(max=100)])
